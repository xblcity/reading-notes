...
`output.publicPath` 配置发布到线上资源的 URL 前缀，为`string` 类型。 默认值是空字符串 ''，可以使用绝对路径或相对路径
...
`preset-env`插件包含了`babel-preset-es2015` `babel-preset-es2016` `babel-preset-es2017` `babel-preset-latest` 插件全部